...
Outlook 批量邮件
改进点：
1) **懒加载 win32com**：不在启动时导入，点“开始批量发送”时再导入，大幅减少首次启动耗时。
2) 其他 UI 保持 v7：主题命名 .msg、较大的输入字体、布局密度/缩放等。

注：若仍觉得启动慢，建议用 PyInstaller 的 --onedir 构建（见说明）。
"""
import csv
import re
import sys
...
def safe_name(s: str) -> str:
    import re as _re
    return _re.sub(r'[\\/:*?"<>|]', "_", (s or "")).strip()[:150] or "email"
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Outlook 批量发送（医院模板 v8 — 启动加速）")
    ...
    # ---------- 执行发送 ----------
    def run(self):
        # 在真正需要时再导入 win32，缩短启动时间
        try:
            _ensure_win32()
        except Exception:
            messagebox.showerror("缺少依赖", "未能导入 pywin32（win32com）。请先安装 pywin32 后再打包 EXE。")
            return
