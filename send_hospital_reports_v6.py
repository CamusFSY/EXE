...
# -*- coding: utf-8 -*-
"""
Outlook 批量邮件（医院模板版）— v6（紧凑布局）
在 v5 的基础上新增“布局密度”选项（紧凑/标准/宽松），可即时切换：
- “紧凑”模式：更小的控件间距、更窄的输入框、更小的整体缩放、窗口宽度限制，布局更紧凑。
- “标准/宽松”模式：逐步放大字体和间距。
仍保留：高DPI适配、滚动区域、浏览文件夹、段落标记、保存.msg、不写日志等。
...
def send_mail(outlook, to_addr, cc, bcc, subject, html_body, attachments, mode, save_msg, msg_dir: Path, msg_prefix: str):
    ...
    if save_msg and msg_dir:
        ...
        msg_path = msg_dir / f"{msg_prefix}_{ts}_{safe_name(subject)}.msg"
        try:
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Outlook 批量发送（医院模板 v6 — 紧凑布局）")
        ...
        self.scale_var = tk.DoubleVar(value=1.0)   # 总体缩放
        ...
        self.build_ui()
        self.apply_density_layout()  # 根据“紧凑/标准/宽松”调整控件尺寸和列权重
    ...
    def apply_style(self):
        ...
        font_size = max(9, int(self.base_font_size * s))
        big_font = (self.base_font_family, font_size + 1, "bold")
        base_font = (self.base_font_family, font_size)
        ...
        self.style.configure("TEntry", padding=pad_entry)
        self.style.configure("TLabel", padding=pad_lbl)
        self.style.configure("TLabelframe", padding=pad_group)
        self.style.configure("TLabelframe.Label", font=big_font)
    ...
    def build_ui(self):
        ...
        self.sec1.grid_columnconfigure(1, weight=0)  # 在紧凑模式下不拉伸
        ...
        for i, it in enumerate(items, start=1):
            ...
            try:
                ...
                res, _ = send_mail(outlook, to_addr, cc, bcc, subject, html, attachments,
                                   self.mode.get(), save_msg, msg_dir, msg_prefix=safe_name(hospital) or "Mail")
