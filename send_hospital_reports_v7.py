...
Outlook 批量邮件
更新：
1) 保存的 .msg 文件名 = “邮件主题”本身（自动清理非法字符；若重名则追加 (1)/(2)... 以避免覆盖）。
2) 界面输入框（Entry）与列表（Listbox）的显示字体“稍微更大”，同时保留“布局密度/界面缩放”可调。
   - 采用 ttk 样式为 Entry 应用更大的字体；Listbox 单独设定字体。
其余功能同 v6：高 DPI 适配、滚动、浏览文件夹、段落标记、保存 .msg、不写日志等。
"""
import csv
import re
import sys
...
OL_MSG = 3  # olMSG；若有编码问题可改 9 = olMSGUnicode
...
def unique_path_by_subject(msg_dir: Path, subject: str) -> Path:
    """以“邮件主题”为基础构造 .msg 文件名；若已存在，追加 (1)/(2)…"""
...
def send_mail(outlook, to_addr, cc, bcc, subject, html_body, attachments, mode, save_msg, msg_dir: Path):
    ...
    if save_msg and msg_dir:
        ...
        msg_path = unique_path_by_subject(msg_dir, subject)
        try:
            mail.SaveAs(str(msg_path), OL_MSG)
        except Exception:
            try:
                mail.SaveAs(str(msg_path), 9)  # olMSGUnicode
            except Exception:
                pass
        else:
            saved = msg_path
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Outlook 批量发送（医院模板 v7 — 主题命名 & 大号输入字体）")
        ...
        self.base_font_size = 12  # 基础比 v6 略大
        self.scale_var = tk.DoubleVar(value=1.05)   # 略微放大默认缩放
        self.density_var = tk.StringVar(value="紧凑")  # "紧凑" / "标准" / "宽松"
        ...
        self.font_entry = tkfont.Font(family=self.base_font_family, size=self.base_font_size + 2)  # Entry 更大一点
        self.font_list = tkfont.Font(family=self.base_font_family, size=self.base_font_size + 2)   # Listbox 更大一点

        self.apply_style()  # 初次应用
        ...
        self.mode = tk.StringVar(value="draft")
        ...
        self.build_ui()
        self.apply_density_layout()   # 根据“紧凑/标准/宽松”微调尺寸
        self.apply_runtime_widget_fonts()  # 对 Listbox 等非 ttk 控件应用字体
    ...
    def apply_style(self):
        ...
        self.style.configure("TEntry", padding=pad_entry, font=self.font_entry)  # Entry 使用更大字体
    ...
    def apply_runtime_widget_fonts(self):
        if hasattr(self, "file_list"):
            try:
                self.file_list.configure(font=self.font_list)
            except Exception:
                pass
    ...
    def build_ui(self):
        ...
        ttk.Checkbutton(save_box, text="发送/草稿均保存 .msg 到下方目录", variable=self.save_msg).pack(side="left")
        ...
        ttk.Radiobutton(mode_box, text="保存到草稿（建议先检查）", variable=self.mode, value="draft").pack(side="left", padx=(0,10))
        ttk.Radiobutton(mode_box, text="直接发送", variable=self.mode, value="send").pack(side="left")
    ...
    def run(self):
        if win32 is None:
            messagebox.showerror("缺少依赖", "未检测到 pywin32。请先安装 pywin32 后再打包 EXE。")
            return
        ...
        msg_dir = Path(self.msg_dir.get().strip()) if save_msg else None
        if save_msg and not self.msg_dir.get().strip():
            messagebox.showerror("保存 .msg", "已勾选保存 .msg，但未指定目录。")
            return
        ...
        for i, it in enumerate(items, start=1):
            ...
            try:
                ...
                res, _ = send_mail(outlook, to_addr, cc, bcc, subject, html, attachments,
                                   self.mode.get(), save_msg, msg_dir)
                if res in ("Sent", "Saved to Drafts"): ok += 1
            except Exception as ex:
